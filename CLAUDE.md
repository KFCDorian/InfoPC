...
- `Sources/InfoPC/` — l'app MenuBarExtra :
  ...
  - `Localization.swift` — langue de l'interface (français / anglais), réglée dans le menu engrenage et persistée dans UserDefaults (`language`) ; à défaut de choix, on suit `Locale.preferredLanguages`. Les textes s'écrivent sur place avec `t("fr", "en")` plutôt que dans un catalogue de clés — à cette échelle, une clé à chercher ailleurs coûte plus qu'elle ne rapporte. **Toute vue qui affiche du texte doit observer `Localization.shared`** (`@ObservedObject private var loc`), sinon elle ne se retraduit pas à la bascule. `t()` est `@MainActor` : ce qui tourne hors du thread principal (installation du helper) renvoie un cas d'échec, traduit par l'appelant (`HelperFailure.message`).
